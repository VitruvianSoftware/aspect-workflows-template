...
## Keeping BUILD files tidy

Bazel `BUILD` files are generated by gazelle and formatted by buildifier. To do
both — plus refresh the Python deps manifest — in a single step, run:

```bash
bazel run //:tidy
```

`//:tidy` runs, in order: `//:gazelle` (regenerate/update `BUILD` files), the
Python deps-manifest update (only when Python is enabled), and `//tools/format`
(buildifier plus every per-language formatter; present when the `lint` feature
is enabled).

- **Pre-commit:** the `githooks/pre-commit` hook runs gazelle and fails the
  commit if any `BUILD` file changes, so stale `BUILD` files are caught locally
  before they land. Bypass with `git commit --no-verify` only when necessary.
- **CI:** the **Tidy Check** workflow (`.github/workflows/tidy-check.yaml`) runs
  `//:tidy` on every pull request and fails if the tree is stale or unformatted.
  The remediation is always the same — run `bazel run //:tidy` and commit the
  result.
